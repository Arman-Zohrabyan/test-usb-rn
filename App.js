...
class App extends Component {
  ...
  stopUsbListener = async () => {
    ...
    if (this.state.selectedDevice) {
      isOpen = await RNSerialport.isOpen(this.state.selectedDevice.name);
    }
    if (isOpen) {
  ...
  handleSendWriteHexString() {
    if (this.state.selectedDevice) {
      RNSerialport.writeBytes(
        this.state.selectedDevice.name,
        this.state.sendText,
      );
    }
  ...
  handleSendWriteBase64() {
    if (this.state.selectedDevice) {
      RNSerialport.writeBase64(
        this.state.selectedDevice.name,
        this.state.sendText,
      );
    }
  ...
  handleSendWriteBytes() {
    if (this.state.selectedDevice) {
      try {
        const array = JSON.parse(this.state.sendText);
        RNSerialport.writeHexString(this.state.selectedDevice.name, array);
      } catch {
        Alert.alert('wrong data, send array. for example: "[1, 2, 3]"');
      }
    }
  }
  ...
  checkSupport() {
    ...
    RNSerialport.isSupported(this.state.selectedDevice.name)
      .then(status => {
        Alert.alert(status ? 'Supported' : 'Not Supported');
      })
      .catch(error => {
        ...
        <View style={styles.container}>
          ...
          <View style={styles.line2}>
            <TouchableOpacity
              ...
              onPress={() => this.handleSendWriteString()}
              disabled={!this.state.connected}>
              <Text style={styles.buttonText}>Send writeString</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.button}
          ...
          <View style={styles.line2}>
            <TouchableOpacity
              style={this.buttonStyle(this.state.connected)}
              onPress={() => this.handleSendWriteBytes()}
              disabled={!this.state.connected}>
              <Text style={styles.buttonText}>Send writeBytes</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={this.buttonStyle(this.state.connected)}
              onPress={() => this.handleSendWriteBase64()}
              disabled={!this.state.connected}>
              <Text style={styles.buttonText}>Send writeBase64</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={this.buttonStyle(this.state.connected)}
              onPress={() => this.handleSendWriteHexString()}
              disabled={!this.state.connected}>
              <Text style={styles.buttonText}>Send writeHexString</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.inputContainer}>
            ...
            <Picker
              enabled={
                this.state.deviceList.length > 1 && !this.state.connected
              }
